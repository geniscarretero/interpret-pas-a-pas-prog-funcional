...
Per modelar el comportament del monticle de memòria conservant la puresa referencial del llenguatge amfitrió, s'ha definit un sistema d'adreçament lògic indexat a \texttt{HSDef.hs}:
...
L'ús de \texttt{Data.IntMap} (un mapa on les claus són exclusivament de tipus \texttt{Int}) com a base per representar el graf aporta avantatges arquitectònics molt significatius en comparació amb estructures d'arbres balançats genèrics o llistes de cerca lineals:
\begin{enumerate}
...
L'estratègia d'avaluació de l'intèrpret segueix un model d'\textbf{avaluació mandrosa} o \textit{lazy evaluation}, formalitzada teòricament mitjançant la reducció d'ordre normal \cite{peyton1987implementation}. Sota aquest model, les expressions no s'avaluen en el moment de ser passades com a arguments d'una funció, sinó únicament i exclusiva quan el seu valor numèric o lògic és estrictament demandat per una operació primitiva primària.

L'estat d'estabilitat d'un node es determina mitjançant el concepte de \textbf{Forma Normal de Cap Feble} o \textbf{WHNF} (\textit{Weak Head Normal Form}). La funció \texttt{isWHNF} codifica aquesta condició de parada clau del sistema:
...
\begin{itemize}
    ...
    \item \textbf{Avaluació de Primitives (\texttt{NVar s} apuntant a un \texttt{PrimDef}):} S'extreuen de la pila els nodes d'aplicació necessaris segons l'aritat de l'operador. Atès que les primitives d'aquest intèrpret (com \texttt{Plus} o \texttt{Mult}) són \textbf{estrictes}, es requereix forçar l'avaluació prèvia dels seus arguments abans de realitzar el càlcul matemàtic. La funció comprova si els fills estan en WHNF; si no ho estan, crida recursivament a \texttt{pas} focalitzant-se en la posició de l'argument desprotegit. Un cop ambdós operands han col·lapsat cap a nodes \texttt{NVal}, s'invoca la funció de càlcul aritmètic, es converteix el resultat de nou a subgraf i s'actualitza el node d'aplicació arrel de l'operació per contenir el valor final.
\end{itemize}

Aquest disseny d'actualitzacions \textit{in situ} sobre l'\texttt{IntMap} d'estat pur garanteix l'harmonia absoluta entre la immutabilitat lògica del llenguatge de programació Haskell i l'eficiència pràctica d'una màquina de reducció de grafs. 

\subsection{Cas Pràctic: Avaluació Pas a Pas de l'Expressió Guia}

Per il·lustrar de manera didàctica l'aplicació d'aquests mecanismes, s'analitza a continuació l'execució de la nostra expressió guia: \texttt{(\textbackslash x y -> x * y) 2 3}. Al mòdul \texttt{HSEval.hs}, la funció \texttt{hsprint} està programada per traduir l'estat del \texttt{HeapState} intern a una cadena de text llegible a cada pas del bucle d'avaluació, oferint a l'usuari una visió transparent del motor de reducció.

La consola de l'intèrpret mostra la següent traça d'estats successius:

\begin{lstlisting}[style=haskellstyle, caption=Traça d'avaluació de la currificació i reducció de l'expressió guia]
bhct> (\x y -> x * y) 2 3
(\x y -> (*) x y) 2 3
(\y -> (*) 2 y) 3
(*) 2 3
...
Aquesta seqüència reflecteix fidelment el recorregut teòric presentat:

\begin{enumerate}
    \item \textbf{Instanciació Inicial:} La cadena original \texttt{(\textbackslash x y -> x * y) 2 3} passa per l'AST i es bolca al graf. S'observa com l'operador infix \texttt{*} ha estat reconegut com una aplicació predefinida associada a l'esquerra: \texttt{(*) x y}.
    \item \textbf{Primera Beta-reducció (\textit{Spine Unwinding}):} L'intèrpret descendeix pel camí esquerre dels nodes d'aplicació fins arribar a la lambda exterior \texttt{\textbackslash x}. Al cim de la pila d'arguments (\texttt{stk}) hi troba el valor \texttt{2}. En aquest pas, substitueix de manera estructural totes les instàncies del node lligat a la \texttt{x} pel node literal \texttt{2}. El node d'aplicació arrel d'aquesta unió es reescriu amb el nou cos generat. El graf intern ara reflecteix la clàusula currificada parcialment avaluada: \texttt{(\textbackslash y -> (*) 2 y) 3}.
    \item \textbf{Segona Beta-reducció:} El mecanisme de l'espinada esquerra es repeteix per a l'aplicació restant. Ara es descendeix fins a \texttt{\textbackslash y} i se li assigna el valor que espera al cim de la pila, un \texttt{3}. La nova reescriptura del graf col·lapsa l'abstracció i deixa aïllada l'expressió \texttt{(*) 2 3}.
    \item \textbf{Execució Estricta de la Primitiva:} S'assoleix el node corresponent a \texttt{(*)}, el qual està lligat a una primitiva aritmètica \texttt{Mult} amb aritat 2 dins del \texttt{preludeDefEnv}. Com que l'operador demana l'avaluació estricta, s'assegura que els arguments (adreces de memòria dels nodes \texttt{2} i \texttt{3}) ja es trobin en \textit{WHNF} (com són \texttt{NVal}, ho estan). S'executa l'operació numèrica a la CPU subjacent i el graf se sobreescriu amb l'únic node atòmic resultant \texttt{NVal 6}. Com que ja no queden més aplicacions a resoldre i el graf compleix la condició total d'estabilitat (\texttt{isWHNF}), l'intèrpret atura el cicle i imprimeix el resultat final: \texttt{6}.
\end{enumerate}